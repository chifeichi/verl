...
class vLLMPDReplica(vLLMReplica):
    ...
    async def launch_servers(self):
        ...
        if use_ascend_layerwise and transfer_backend == "nixl":
            logger.warning("NixlConnector is unavailable")
    ...
    def _build_kv_transfer_config(
        ...
        if use_ascend_layerwise:
            if transfer_backend != "mooncake":
                raise ValueError("Ascend vllm pd requires transfer_backend=mooncake")
            if kv_port is None or prefill_tp is None or decode_tp is None:
                raise ValueError("Ascend MooncakeLayerwiseConnector requires kv_port, prefill_tp, decode_tp")
